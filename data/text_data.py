...
class WikiTextDataset(DatasetWrapper):
    ...
    def combine_dataset(self, data):
        # WikiText article assembly depends on headings across the whole split.
        # Independent map batches would reset combine_func state at artificial
        # boundaries and silently split articles.
        return data.map(lambda x: self.combine_func(x), batched=True, batch_size=None, remove_columns=data.column_names, num_proc=None, desc="Combining")
